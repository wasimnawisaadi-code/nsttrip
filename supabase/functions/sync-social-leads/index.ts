// High-Performance Social Leads Sync (Optimized)
...
Deno.serve(async (req) => {
  ...
  // 1. Fetch Admins ONCE to save time
  ...
  for (const source of Object.keys(SHEETS) as Array<keyof typeof SHEETS>) {
    try {
      ...
      let headerIdx = allRows.findIndex(r => r.some(c => (c || "").toLowerCase().includes("contact id") || (c || "").toLowerCase().includes("first name")));
      ...
      for (const r of dataRows) {
        ...
        const lead = { source, unique_key: id, full_name: name, phone: phone || null, username: username || null, raw: r, updated_at: new Date().toISOString() };
        ...
        if (existing) { await supabase.from("social_leads").update(lead).eq("id", existing.id); summary[source].updated++; }
          ...
          if (ins) {
            ...
            adminIds.forEach(uid => {
              pendingNotifications.push({ user_id: uid, title: `New ${source} lead`, message: `${ins.full_name} messaged.`, type: "lead" });
            });
          }
        }
      }
  ...
  // 2. Bulk Insert all notifications at the end (MUCH FASTER)
  if (pendingNotifications.length > 0) {
    await supabase.from("notifications").insert(pendingNotifications.slice(0, 100));
  }
